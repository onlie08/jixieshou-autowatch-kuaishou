    ...
    public void startTask(final AppInfo appInfo) {
        ...
        if (scriptThread == null) {
            ...
            scriptThread = new Thread(() -> {
                while (!scriptThread.interrupted()) {
                    try {
                        IScript script = null;
                        ...
                        if (script != null) {
                            ...
                            runOnUiThread(() -> BusManager.getBus().post(new BusEvent<>(EventType.start_task, allTime)));

                            while (System.currentTimeMillis() - st < allTime && !scriptThread.interrupted()) {
                                try {
                            ...
                            runOnUiThread(() -> BusManager.getBus().post(new BusEvent<>(EventType.task_finish, currentTaskApp)));
